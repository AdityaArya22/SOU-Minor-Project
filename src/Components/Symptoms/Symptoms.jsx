...
import diseaseData from "../MultistepForm/diseases.json"; // Disease data
...
const Symptoms = () => {
  ...
  const handleSubmit = () => {
    ...
    const exactMatches = dieseases.filter((disease) => {
      ...
      return matchedSymptoms.length >= 2; // At least two symptoms for exact match
    });
    ...
    const partialMatches = dieseases.filter((disease) => {
      ...
      return matchedSymptoms.length > 0 && matchedSymptoms.length < 2; // One symptom for partial match
    });
    ...
    if (allMatches.length > 0) {
      allMatches.sort((a, b) => {
        const aMatches = a.commonSymptoms.filter((symptom) =>
          state.symptoms.includes(symptom)
        ).length;
        const bMatches = b.commonSymptoms.filter((symptom) =>
          state.symptoms.includes(symptom)
        ).length;
      ...
      dispatch(matchedSymtomps(top3Diseases));
      navigate("/result");
    } else {
